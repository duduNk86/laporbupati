...
class Aduan extends CI_Controller
{
	...
	function kirim_aduan_disabilitas()
	{
		...
		if (!empty($_FILES['x_audio']['name'])) {
			...
			if ($this->upload->do_upload('x_audio')) {
				...
				$this->session->set_flashdata('sukses', '[Laporan Anda berhasil dikirim! silahkan tunggu Link Tracking untuk melihat Progres Tindaklanjutnya yang akan dikirimkan melalui nomor HP/WA yang telah disampaikan]');
				redirect('home');
			} else {
	...
	// 			$this->output->set_header('HTTP/1.0 200 Ok');
	// 			// $this->session->set_flashdata('sukses','Laporan Anda sudah terkirim, silahkan menunggu pemberitahuan selanjutnya melalui email/kontak hp yang tercantum');
	// 			// redirect('');
	...
	function kirim_rating_jawaban_pelapor()
	{
		$recaptcha = $this->input->post('g-recaptcha-response');
		$id = $this->input->post('xkode_detail');
		$rating_jawaban = $this->input->post('selected_rating1');
		if (!empty($recaptcha)) {
			$data = array(
				'id' => $id,
				'rating_jawaban' => $rating_jawaban
			...
			$this->session->set_flashdata('success', 'Jawaban Anda sudah terkirim, terimakasih telah mengisi Survei. Jawaban Anda akan membantu kami dalam upaya untuk meningkatkan Layanan Lapor Bupati Wonosobo.');
			sleep(3); //tunggu 3 detik untuk redirect ke halaman home
			redirect('home');
		} else {
			echo $this->session->set_flashdata('error', '<div class="alert alert-danger" role="alert"><button type="button" class="close" data-dismiss="alert"><span class="fa fa-close"></span></button>Isikan Captcha !!</a></div>');
			redirect('home');
		}
	}
